...
public class Paxel extends Item {
    ...
    public static TagKey<Block> paxelMineable = ModTags.Blocks.PAXEL_MINEABLE;
    private final ToolMaterial tier;

    public Paxel(ToolMaterial tier, float attackDamage, float attackSpeed, Settings settings) {
        super(computeSettings(tier, ModTags.Blocks.PAXEL_MINEABLE, attackDamage, attackSpeed, settings));
        paxelMineable = ModTags.Blocks.PAXEL_MINEABLE;
        this.tier = tier;
    }
    ...
    private static Settings computeSettings(ToolMaterial tier, TagKey<Block> paxelMineable, float attackDamage, float attackSpeed, Settings settings) {
        ...
        settings.pickaxe(wrapMaterial(tier, tier.durability()), attackDamage, attackSpeed);
        settings.shovel(wrapMaterial(tier, tier.durability()), attackDamage, attackSpeed);
        settings.axe(wrapMaterial(tier, tier.durability()), attackDamage, attackSpeed);
        settings.tool(tier, BlockTags.PICKAXE_MINEABLE, attackDamage, attackSpeed, 0.0F);
        return settings;
    }
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (!world.isClient()) {
            ...
            if (player != null) context.getStack().damage(1, player, Hand.MAIN_HAND);
        }
