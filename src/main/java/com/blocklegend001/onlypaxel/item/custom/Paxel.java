...
import com.blocklegend001.onlypaxel.item.ModToolMaterials;
...
public class Paxel extends MiningToolItem {
    ...
    public Paxel(ToolMaterial tier, float attackDamage, float attackSpeed) {
        super(tier, ModTags.Blocks.PAXEL_MINEABLE, createProperties(tier, attackDamage, attackSpeed));
    }

    private static Settings createProperties(ToolMaterial tier, float attackDamage, float attackSpeed) {
        Settings properties = new Settings()
                .attributeModifiers(Paxel.createAttributeModifiers(tier, attackDamage, attackSpeed))
                .component(DataComponentTypes.TOOL, tier.createComponent(ModTags.Blocks.PAXEL_MINEABLE));

        if (tier == ModToolMaterials.NETHERITE_PAXEL) {
